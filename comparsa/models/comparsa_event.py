...
class ComparsaEvent(models.Model):
  ...
  @api.constrains("pricing_mode", "price_member", "price_guest", "price_children")
  def _check_prices(self):
    for rec in self:
      if rec.pricing_mode == "free":
        ...
        if rec.price_member <= 0 or rec.price_guest <= 0 or rec.price_children <= 0:
          raise ValidationError("Los precios deben ser mayores que 0 cuando el modo de precio es fijo.")
